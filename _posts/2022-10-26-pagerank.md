...
Second, given that $$1$$ is an eigenvector, then we can solve $$A r = r$$ using a row-reduction algorithm such as [Gaussian elimination](https://en.wikipedia.org/wiki/Gaussian_elimination). Is that it? The answer is no, because Gaussian elimination has the time complexity of $$O(n^3)$$, where $$n$$ is the number of pages. This does not scale well with our page collection, because $$n$$ could be in the billions, if not more. Therefore, we need to find another way to solve $$\eqref{eq:fixed_point}$$.
...
<div style="padding-left:2em; padding-right:2em">
    ...
    \end{align*}
    $$
    where the first inequality uses the triangle inequality and the definition of $x_j$, and the last equality uses the fact the column $j$ of $A$ sums to 1. This implies that $|\lambda| \leq 1$.
</div>
